...
public class SynAnalyzer {
    ...
    /******************
     *	Constructor for SynAnalyzer, initializes Scanner object and tokenArray
     *    @author - Geoff Wellington
     *	@date - 06/19/10
     *******************/
    ...
    /******************
     *	Checks to see that the token passed in is the same as the current token
     *  @author - Geoff Wellington
     *	@date - 06/19/10
     *******************/
    ...
    private void factor() {
        if (checkToken("23"))        //check for (
        {
            expression();
            expect("24");                //check for )
        } else if (!checkIdentifier(token) && !checkNum(token)) {
            error("factor");
        }
    }
}